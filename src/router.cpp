...
#define PACKET_BUFFER_SIZE 5 //for second scenario
#define MIN 11
#define MAX 20
...
int dropProbability = 10; //for second scenario
...
struct Packet {
	...
	bool is_ack; 
	int SN; // packet id or sequence numberp
	bool src; // source address: 0 for A and 1 for B
	bool dest; //destination address: 0 for A and 1 for B
	string body;
};
...
void* sendPackets(void* args) {
	while(true) {
		...
		if(p.dest) { //sending packet to B
			std::cout << "forwarding packet " << p.SN << " from A to B...\n";
...
void addPacket(string mssg1, string mssg2, string mssg3, int sender, char recBuffer[]) {
	...
	if(packetsBuffer.size() < PACKET_BUFFER_SIZE) {
		...
		packetsBuffer.push(Packet(0, frameSN, sender, !sender, convertToString(temp, size)));
