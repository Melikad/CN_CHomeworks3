...
int last_generated_link = 0;

int globalIdGenerator(){
    return ++last_generated_link;
}

struct Node{
    Node(string ip_, bool type_): ip(ip_), type(type_){
...
struct Link {
    Link(Node* ip1_, Node* ip2_, int cost_): first(ip1_), second(ip2_), cost(cost_) {};
...
public:
    ...
    void lsrp(int src);
    void lsrpAll();
    void dvrp(string src_);
    void dvrpAll();
    void removeLink(string input);
    void updateCommands(string input);
    void addCommands(string input);
...
private:
    ...
    Node* get_node (string ip);
    vector<string> getArgs(string args);
...
Node* NetWork::get_node(string ip){
...
bool NetWork::isValidIp(string ip)
{
    for (int i = 0; i < hosts.size(); i++)
        if (hosts[i].ip == ip)
            return true;
...
void NetWork::dvrp(string src_)
{
    int src = get_node(src_)->id;
    auto start = high_resolution_clock::now();
    auto mid = high_resolution_clock::now();
...
vector<string> NetWork::getArgs(string args)
{
    string u;
    stringstream ls(args);
    vector<string> vec;
...
void NetWork::addCommands(string input)
{
    vector<string> args = getArgs(input);
    if (args[0] == "hosts"){
        for (int i = 1; i < args.size(); i++){
            if (isValidIp(args[i])){
                cout << "Skipping duplicate host" << endl;
                continue;
            }
            hosts.push_back(Node(args[i], 0));
        }
    }
    if (args[0] == "routers"){
        for (int i = 1; i < args.size(); i++){
            if (isValidIp(args[i])){
                cout << "Skipping duplicate router" << endl;
                continue;
            }
            routers.push_back(Node(args[i], 0));
        }
    }  
    if (args[0] == "link"){
        if (args.size() != 4){
        ...
        if (isValidIp(args[1]) && isValidIp(args[2])){
            links.push_back(Link(get_node(args[1]), get_node(args[2]), stoi(args[3])));
        }
    }   
}
    ...
    if (args[0] == "link"){
        ...
        if (isValidIp(args[1]) && isValidIp(args[2])){
            ...
            for (int i = 0; i < links.size(); i++){
                if (links[i].first->ip == args[1] && links[i].second->ip == args[2]){
                    index = i;
                    break;
                }
            }
...
void NetWork::updateCommands(string input)
{
    vector<string> args = getArgs(input);
    if (args[0] == "link")
    {
...
void handleCommands(NetWork *network)
{
    ...
    while (cin >> command)
    {
        ...
        if (command == "update")
            network->updateCommands(args);
        if (command == "add")
            network-> addCommands(args);
    }
}
